    ...
    while read -u 3 FILESTR; do
		for AFILE in $FILESTR; do
			...
			if [ -f "$AFILE" ]; then
				...
				tryTo sudo lndir -withrevinfo "$AFILE" "$ARCHIVEDIR/$AFILE"
...
compareSystemFiles(){
	...
	while read -u 3 ASYSFILE; do
		case "$ASYSFILE" in
			./sysarchive-*.lst.txt | ./rcDirs.txt | ./links.txt | ./autosource)
